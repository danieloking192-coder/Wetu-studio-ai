...
def test_image_signature_and_hash(tmp_path):
    p=tmp_path/"x.jpg"; p.write_bytes(b"\xff\xd8\xff"+b"x"*20)
...
def test_bad_signature_is_blocked(tmp_path):
    p=tmp_path/"x.jpg"; p.write_bytes(b"not-jpeg")
    try: MediaSecurityScanner(require_antivirus=False).scan(p,"image/jpeg")
    except ValueError: pass
    else: raise AssertionError("spoofed MIME must be blocked")

def test_malware_required_blocks_without_scanner(tmp_path):
    p=tmp_path/"x.jpg"; p.write_bytes(b"\xff\xd8\xff"+b"x"*20)
    try: MediaSecurityScanner(require_antivirus=True).scan(p,"image/jpeg")
    except RuntimeError: pass
    else: raise AssertionError("required AV must fail closed")
