...
def test_photo_becomes_persistent_character_reference(tmp_path):
    ...
    item=store.import_image("hero-01","portrait.jpg","image/jpeg",base64.b64encode(b"fake-image").decode(),True)
...
def test_real_person_requires_consent(tmp_path):
    store=CharacterIdentityStore(tmp_path/"identity")
    try: store.import_image("hero-01","portrait.jpg","image/jpeg",base64.b64encode(b"x").decode(),False)
