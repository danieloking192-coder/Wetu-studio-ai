...
Fuses true-story provenance, emotion/atmosphere, persistent Character DNA,
World DNA and Scene/Production Memory into one provider-neutral scene context.
This layer does not render media; it prepares and validates the contract that
a renderer must preserve.
"""
...
from .emotion_atmosphere import EmotionAtmosphereEngine, SceneAtmosphere
from .true_story_realism import TrueStoryRealismEngine, TrueStoryMode, TrueStoryScene
...
class ProductionRealismOrchestrator:
    ...
    def __init__(
        self,
        *,
        emotion: EmotionAtmosphereEngine | None = None,
        true_story: TrueStoryRealismEngine | None = None,
    ) -> None:
        ...
        *,
        ...
        if true_story:
            ts = self.true_story.build(
                scene_id=scene_id,
                event=str(true_story.get("event", "")),
    ...
    def validate(self, context: Dict[str, Any]) -> List[str]:
        ...
        if emotion_raw:
            emotion_scene = self.emotion.build(
                scene_id=emotion_raw["scene_id"],
                mood=emotion_raw["mood"],
                ...
                source_vs_interpretation=emotion_raw.get(
                    "source_vs_interpretation", "artistic_direction"
                ),
            )
